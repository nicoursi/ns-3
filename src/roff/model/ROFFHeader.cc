...
ROFFHeader::ROFFHeader (uint32_t                type,
                        Vector                  position,
                        uint32_t                sender,
...
uint32_t
ROFFHeader::GetType () const
{
  return m_type;
}

Vector
ROFFHeader::GetStarterPosition () const
{
  return m_starterPosition;
}

uint32_t
ROFFHeader::GetSenderId () const
{
  return m_senderId;
}

const Vector&
ROFFHeader::GetPosition () const
{
  return m_position;
}

boost::dynamic_bitset<>
ROFFHeader::GetESDBitmap () const
{
  return m_esdBitmap;
}

uint32_t
ROFFHeader::GetPhase () const
{
  return m_phase;
}

uint32_t
ROFFHeader::GetSlot () const
{
  return m_slot;
}

uint8_t
ROFFHeader::IsSenderInJunction () const
{
...
uint64_t
ROFFHeader::GetJunctionId () const
{
  NS_LOG_FUNCTION (this);
  return m_junctionId;
}
...
void
ROFFHeader::SetType (uint32_t type)
{
  m_type = type;
}

void
ROFFHeader::SetSenderId (uint32_t senderId)
{
  m_senderId = senderId;
}

void
ROFFHeader::SetStarterPosition (const Vector& position)
{
  m_starterPosition = position;
}

void
ROFFHeader::SetPosition (const Vector& position)
{
  m_position = position;
}

void
ROFFHeader::SetESDBitmap (const boost::dynamic_bitset<>& esdBitmap)
{
  m_esdBitmap = esdBitmap;
}

void
ROFFHeader::SetPhase (uint32_t phase)
{
  m_phase = phase;
}

void
ROFFHeader::SetSlot (uint32_t slot)
{
  m_slot = slot;
}

void
ROFFHeader::SetSenderInJunction (uint8_t value)
{
  NS_LOG_FUNCTION (this);
...
void
ROFFHeader::SetJunctionId (uint64_t junctionId)
{
  NS_LOG_FUNCTION (this);
  m_junctionId = junctionId;
}
...
TypeId
ROFFHeader::GetInstanceTypeId () const
{
  return GetTypeId ();
}

double
ROFFHeader::GetESDBitmapRoundedSizeInBytes (uint32_t bitmapSize) const
{
...
uint32_t
ROFFHeader::GetSerializedSize () const
{
...
void
ROFFHeader::WriteDouble (Buffer::Iterator* iter, double d) const
{
  const double*  buf  = &(d);
...
void
ROFFHeader::WriteESDBitmap (Buffer::Iterator* iter) const
{
  NS_LOG_FUNCTION (this);
  ...
  // iter->Write (&esdBitmapToExtend, calcolareSize) nb <<= 30;
  // cout << "ROFFHeader::WriteESDBitmap after " << nb << endl;
}

void
ROFFHeader::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION (this);
...
double
ROFFHeader::ReadDouble (Buffer::Iterator* iter) const
{
  uint64_t x;
...
void
ROFFHeader::ConcatBitsets (boost::dynamic_bitset<>&       a,
...
void
ROFFHeader::ReadESDBitmap (Buffer::Iterator* iter, uint32_t bitmapSize)
{
  // bitmapSize is a multiple of 8

  uint32_t bytesToRead = bitmapSize / 8;

  // cout << "ROFFHeader::ReadESDBitmap bitmapSize= " << bitmapSize
  //      << " bytesToRead= " << bytesToRead << endl;
...
uint32_t
ROFFHeader::Deserialize (Buffer::Iterator start)
{
  NS_LOG_FUNCTION (this);
...
void
ROFFHeader::Print (ostream& os) const
{
  // todo complete this
  NS_LOG_FUNCTION (this);
  os << "m_type (" << m_type << ") "
