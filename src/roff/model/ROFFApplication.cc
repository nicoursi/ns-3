...
{
  static TypeId tid = TypeId ("ns3::ROFFApplication").SetParent<Object> ().SetGroupName ("Network");
  return tid;
}
      ...
      for (uint32_t i = 1; i < 151; i++)
        {
          ...
          ROFFHeader header (
            headerType, position, forgedSenderNodeId + i, position, boost::dynamic_bitset<> (), 0, 0);
      ...
      if (sentMap.count (send) == 0)
        {
          ...
          Simulator::Schedule (
            MilliSeconds (waitingTime), &ROFFApplication::GenerateHelloMessage, this, roffNode);
        }
    }
...
ROFFApplication::StartBroadcastPhase (void)
{
  ...
  if (m_errorRate > 0)
    {
      Simulator::Schedule (
        MilliSeconds (1), &ROFFApplication::GenerateAlertMessage, this, m_nodes.at (m_startingNode));
    }
}
...
void
ROFFApplication::HandleAlertMessage (Ptr<ROFFNode> node, ROFFHeader header, uint32_t distance)
{
  ...
  else
    {
      ...
      if (phase > node->GetPhase ())
        { //todo abilitare per urbano
          NS_LOG_LOGIC ("node " << node->GetId () << "is not inside a junction: updates phase from "
      ...
      if (node->GetPhase () > phase)
        {
          NS_LOG_DEBUG ("ROFFApplication::ForwardAlertMessage node "
                        << node->GetId () << " pos " << node->UpdatePosition ()
                        << " defers because of phase (" << node->GetPhase () << " > " << phase << ")");
          return;
        }
      if (node->GetSent ())
        {
          NS_LOG_DEBUG ("ROFFApplication::ForwardAlertMessage node " << node->GetId () << " pos "
                                                                     << node->UpdatePosition ()
...
uint32_t
ROFFApplication::ComputeMinDiff (uint32_t distSenderToNode, uint32_t distSenderToAnotherNode)
{
                                      ...
                                      Vector higherPriorityNodeCoords)
{
  ...
  uint32_t minDiff =
    ceil (pdFwdToHigherPriority - pdFwdToLowerPriority + pdHigherToLowerPriority + rxTx + ccaTime);
