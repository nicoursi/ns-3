...
void
NeighborTable::AddOrUpdateEntry (uint32_t nodeId, Vector pos, Time timeStamp)
{
...
uint32_t
NeighborTable::GetNBTSize () const
{
  return m_table.size ();
}
...
uint32_t
NeighborTable::GetMaxDistance (Vector pos) const
{
                                          ...
                                          int32_t& dist) const
{
  ...
  NS_LOG_INFO ("NeighborTable::GetCoordsOfVehicleInRange "
               "Coords not found for vehicle in range= "
               << range << " and node in pos= " << nodePosition);
  dist = -1;
  return Vector ();
}

bool
NeighborTable::IsNodeWinnerInContention (uint32_t id, uint32_t dist, Vector pos) const
{
  for (auto entry : m_table)
    {
      //		cout << "a" << endl;
