...
void
PositionRankingMap::AddEntry (uint32_t index, uint32_t priority)
{
...
uint32_t
PositionRankingMap::GetPriority (uint32_t distance)
{
...
uint32_t
PositionRankingMap::GetUpperDistanceLimit (uint32_t priority)
{
  for (auto pair : m_positionRanking)
    {
...
PositionRankingKey
PositionRankingMap::GetRange (uint32_t priority)
{
  for (auto pair : m_positionRanking)
    {
      if (pair.second == priority)
...
std::ostream&
operator<< (std::ostream& os, const PositionRankingMap& map)
{
