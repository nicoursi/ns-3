...
public:
  ...
  /**
        ...
        * \return none
        */
  void Install (uint32_t broadcastPhaseStart,
...
private:
  ...
  uint32_t
    m_errorRate; //probability to incur in an error in transmission schedule (sending 1 slot earlier or later)
  ...
  vector<uint32_t>
  m_receivedNodes;        // ids of nodes which have received alert messages, duplicates allowed
  ...
  vector<Edge>
  m_transmissionVector;   //vector to discover paths of alert messages (single broadcasts ordered by time of reception)
};
