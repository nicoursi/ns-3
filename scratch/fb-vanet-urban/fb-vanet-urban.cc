...
/**
 * \ingroup obstacle
 * \brief The VanetRoutingExperiment class implements an application that allows this
 * VANET experiment to be simulated
 */
...
public:
  ...
  /**
   * \brief Calculates the filepath where the output of the experiment will be saved
...
private:
  ...
  /**
   * \brief Get the default transmission power for a given range
   *
   * NOTE: m_txp has an impact only when m_propagationLoss == 1,
   * otherwise it will be completely ignored
   *
   * \param actualRange The desired communication range in meters
   * \return double The transmission power in dBm
   * \throws std::invalid_argument if actualRange is not supported
   */
...
FBVanetExperiment::CalculateOutFilePath () const
{
  ...
  // fileName.append ("cw-" + cwMin + "-" + cwMax + "/" + m_mapBaseNameWithoutDistance +
  //                  "/d" + vehicleDistance + "/b" + buildings + "/" + protocol + "-" +
  //                  actualRange + "/" + m_mapBaseName + "-cw-" + cwMin + "-" + cwMax +
  //                  "-b" + buildings + "-" + protocol + "-" + actualRange);
...
double
FBVanetExperiment::GetDefaultTxPower (double actualRange)
{
  double txp;
  ...
  for (uint32_t i = 0; i < m_nNodes; i++)
    {
      ...
      AddressValue remoteAddress (
        InetSocketAddress (ns3::Ipv4Address::GetAny (), m_port));
...
FBVanetExperiment::CommandSetup (int argc, char* argv[])
{
  ...
  cmd.AddValue ("startRun",
                "Run number the simulations will start from. If set will take precedence "
                "over NS_GLOBAL_VALUE=RngRun=n",
                m_startRun);
  cmd.AddValue ("maxRun",
